                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div style="display: flex; justify-content: space-between">
                                    ...
                                    <form class="search-form d-flex align-items-center" method="get" action="searchMajor">
                                ...
                                <tbody>
                                    <c:forEach items="${ListSchedule}" var="x">
                                        <tr>               
                                            ...
                                            <td><c:if test="${x.user.gender == 1}"> Male</c:if>
                                            <c:if test="${x.user.gender == 2}"> Female</c:if></td>
                                            <td>${x.descrip}</td>
                                            <td>${x.major.name}</td>
                                            <td>${x.major.room}</td>
                                            <td>
                                                <a href="/schedule?action=view&id=${x.id}" class="btn btn-info">View</a>
                                                <a href="/schedule?action=view&id=${x.id}" class="btn btn-warning">Update</a>
                                                <a href="/schedule?action=view&id=${x.id}" class="btn btn-danger">Delete</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
