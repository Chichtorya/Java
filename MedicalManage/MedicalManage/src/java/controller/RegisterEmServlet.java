 ...
 * @author dell
 */
@WebServlet(name="RegisterEmServlet", urlPatterns={"/registerEmployee"})
public class RegisterEmServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet RegisterEmServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet RegisterEmServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String room = request.getParameter("major");
        String salary = request.getParameter("salary");
        
        String Password = "abc123";
        if (name.length() > 35 || dao.checkPhone(phone) || dao.checkCitizenId(Citizen)
                || dao.checkHealthId(Health) || Address.length() > 50 || dao.checkGmail(Gmail)) {
            if (name.length() > 35) {
            ...
            try {
                ...
                int major = Integer.parseInt(room);
                ...
                String mess = "<!DOCTYPE html>\n"
                        ...
                        + "        <div>Please check account and change your password.</div>\n"
                        + "    </body>\n"
                        + "</html>";
